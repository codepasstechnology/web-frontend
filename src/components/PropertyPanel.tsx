...
import { X, MapPin, Building2, ShieldCheck, Phone, Gauge, Share2, User, Briefcase, Check } from "lucide-react";
    ...
    <aside className={panelClass}>
      <div className="flex items-start justify-between border-b border-border p-4">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{parcel.parcelNumber} · {parcel.county}</p>
        </div>
        <button onClick={onClose} className="ml-2 rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <div className="mb-4 rounded-md border border-border bg-background p-3">
          <div className="mb-1 flex items-center justify-between">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Development Score</span>
            <Gauge className="h-3.5 w-3.5 text-muted-foreground" />
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-semibold text-foreground">{parcel.amenities.developmentScore}</span>
            <span className="text-xs text-muted-foreground">/ 100</span>
          </div>
          <div className="mt-2 h-1.5 w-full rounded-full bg-muted">
            <div className="h-1.5 rounded-full bg-[var(--accent)]" style={{ width: `${parcel.amenities.developmentScore}%` }} />
          </div>
        </div>
        ...
        <Section title="Location Intelligence">
          <Row icon={<Building2 className="h-3.5 w-3.5" />} label="School" value={parcel.amenities.school} />
          <Row icon={<Building2 className="h-3.5 w-3.5" />} label="Hospital" value={parcel.amenities.hospital} />
          <Row icon={<Building2 className="h-3.5 w-3.5" />} label="Shopping" value={parcel.amenities.shopping} />
          <Row icon={<MapPin className="h-3.5 w-3.5" />} label="Main Road" value={parcel.amenities.mainRoad} />
          <Row icon={<MapPin className="h-3.5 w-3.5" />} label="To Tarmac" value={parcel.amenities.distanceToTarmac} />
        </Section>

        <Section title="Utilities">
          <div className="flex flex-wrap gap-1.5">
            {parcel.amenities.utilities.map((u) => (
              <span key={u} className="rounded-sm border border-border bg-background px-2 py-0.5 text-[11px] text-foreground">{u}</span>
            ))}
          </div>
        </Section>
        ...
        <Section title={parcel.postedBy === "owner" ? "Listed by Owner" : "Listed by Broker"}>
          ...
          <p className="mt-1 inline-flex items-center gap-1 text-xs text-foreground"><Phone className="h-3 w-3" /> {parcel.seller.phone}</p>
        </Section>
      </div>
      ...
      <div className="flex items-center gap-2 border-t border-border p-4">
        ...
        <ShareButton title={parcel.title} text={`${parcel.parcelNumber} · ${parcel.size} · KES ${parcel.price.toLocaleString()}`} />
      </div>
    </aside>
  );
}
    ...
    <aside className={panelClass}>
      <div className="flex items-start justify-between border-b border-border p-4">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{rental.area}, {rental.county}</p>
        </div>
        <button onClick={onClose} className="ml-2 rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground">
          <X className="h-4 w-4" />
        </button>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <Section title={rental.postedBy === "owner" ? "Listed by Owner" : "Listed by Broker"}>
          ...
          <p className="mt-1 inline-flex items-center gap-1 text-xs text-foreground"><Phone className="h-3 w-3" /> {rental.agent.phone}</p>
        </Section>
        <p className="text-sm text-muted-foreground">Verified rental listing within the LandVerify network. Schedule a viewing or request a tenancy report.</p>
      </div>
      <div className="flex items-center gap-2 border-t border-border p-4">
        <button className="flex-1 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-secondary">
          Schedule Viewing
        </button>
        <ShareButton title={rental.title} text={`${rental.type} · ${rental.area} · KES ${rental.price.toLocaleString()}/mo`} />
      </div>
    </aside>
  );
}
