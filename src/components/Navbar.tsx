        ...
        <div className="ml-auto flex items-center gap-1.5 sm:gap-2 lg:ml-0">
          ...
          {user ? (
            <Link
              to="/dashboard"
              search={{ tab: undefined }}
              className="hidden rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-secondary sm:inline-flex"
            >
              Dashboard
            </Link>
          ) : (
            <>
              <Link
                to="/login"
                className="hidden rounded-md border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted sm:inline-flex"
              >
                Login
              </Link>
              <Link
            ...
            <div className="mt-2 flex gap-2 border-t border-border pt-2">
              {user ? (
                <Link
                  ...
                  search={{ tab: undefined }}
                  onClick={() => setMenuOpen(false)}
                  className="flex-1 rounded-md bg-primary px-3 py-1.5 text-center text-xs font-medium text-primary-foreground hover:bg-secondary"
                >
                  Dashboard
                </Link>
              ) : (
                <>
                  <Link
                    to="/login"
                    onClick={() => setMenuOpen(false)}
                    className="flex-1 rounded-md border border-border px-3 py-1.5 text-center text-xs font-medium text-foreground hover:bg-muted"
                  >
                    Login
                  </Link>
                  <Link
