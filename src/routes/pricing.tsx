...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Simple, transparent pricing for land sellers, agents and developers across Kenya." },
      { property: "og:title", content: "Pricing — LandVerify Kenya" },
      { property: "og:description", content: "Free, Basic and Pro plans for listing verified land in Kenya." },
    ],
  }),
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {plans.map((p) => (
            <PlanCard key={p.id} plan={p} current={user?.plan === p.id} onSelect={() => choose(p.id)} />
          ))}
        </div>

        <div className="mt-16">
          <h2 className="text-lg font-semibold text-foreground">Boost Individual Listings</h2>
          <p className="mt-1 text-sm text-muted-foreground">One-time add-ons available on any plan.</p>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {addOns.map((a) => (
              <div key={a.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-5 shadow-sm">
                ...
                <div className="text-right">
                  ...
                  <button className="mt-1 text-xs font-medium text-[#2563EB] hover:underline">Add to listing</button>
                </div>
              </div>
            ))}
          </div>
